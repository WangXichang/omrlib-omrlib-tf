...
def weight_variable(shape):
    ...
    return tf.Variable(initial_value, name='weight')


def bias_variable(shape):
    initial_value=tf.constant(value=0.1, shape=shape)
    return tf.Variable(initial_value, name='bias')


def conv_2d(x, w):
    return tf.nn.conv2d(input=x, filter=w, strides=[2, 3, 1, 1], padding="SAME")
...
def test_cnn(batch_size=30, lr=0.0001, num_iter=2000):
    # prepare omr dataset
    ...
    w_conv1 = weight_variable(shape=[3, 5, 1, 8])
    b_conv1 = bias_variable(shape=[8])
    reshape_x = tf.reshape(x, shape=[-1, 10, 15, 1])  # 省略的形式区别于占位符!!!!!!
    conv1_out = tf.nn.relu(conv_2d(reshape_x, w_conv1)+b_conv1)
    pool1_out = max_pool_2x2(conv1_out)

    w_conv2 = weight_variable(shape=[5, 5, 8, 8])
    b_conv2 = bias_variable(shape=[8])
    conv2_out = tf.nn.relu(conv_2d(pool1_out, w_conv2) + b_conv2)
    # pool2_out = max_pool_2x2(conv2_out)

    full_connected_in = tf.reshape(conv2_out, shape=[-1, 5*5*8])
    # full_connected_in = tf.reshape(pool2_out, shape=[-1, 7*7*8])
    w_full_connected = weight_variable(shape=[5*5*8, 32])
    b_full_connected = bias_variable(shape=[32])
    ...
    w_softmax = weight_variable(shape=[32, 2])
    ...
    for iter in range(num_iter):
        ...
        sess.run(fetches=step_train, feed_dict={x: x_image,
                                                y: y_label,
                                                dropout_prob: 0.5})
